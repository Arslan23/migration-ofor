...
export const DEFAULT_WORKFLOWS: Workflow[] = [
  ...
  },
  {
    ...
    natures: ["sensibilisation", "Sensibilisation", "suivi", "Suivi & Évaluation", "autre"],
    steps: [
...
export const getWorkflowForNature = (nature: string, workflows: Workflow[] = DEFAULT_WORKFLOWS): Workflow => {
  const normalized = nature.toLowerCase().trim();
  return workflows.find(wf => wf.natures.map(n => n.toLowerCase().trim()).includes(normalized)) || workflows.find(wf => wf.code === "WF-SIMPLE")!;
};
